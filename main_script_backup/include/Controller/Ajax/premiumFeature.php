...
class premiumFeature extends AjaxBase
{
    ...
    private function checkGold($featureKey)
    {
        ...
        foreach ($config->extraSettings->buyTroops['packages'][Session::getInstance()->getRace()] as $index => $package) {
            $paymentGolds['buyTroops' . $index] = $package['coins'];
        }
    ...
    private function buyAnimal($featureKey)
    {
        ...
        $units = $config->extraSettings->buyAnimal['packages'][$packageId]['units'];
        array_unshift($units, $units[0]);
        unset($units[0]);
    ...
    private function buyResources($featureKey)
    {
        ...
        if (!isset($config->extraSettings->buyResources['packages'][$packageId])) {
            return;
        }
        ...
        $resources = $config->extraSettings->buyResources['packages'][$packageId]['resources'];
        $db->begin_transaction();
        ...
        $coins = $config->extraSettings->buyResources['packages'][$packageId]['coins'];
        if (GoldHelper::decreaseGold(Session::getInstance()->getPlayerId(), $coins)) {
            $db->commit();
        } else {
            $db->rollback();
    ...
    private function buyTroops($featureKey)
    {
        ...
        if (!isset($config->extraSettings->buyTroops['packages'][$race][$packageId])) {
            return;
        }
        ...
        $this->response['data']['functionToCall'] = 'productPurchased';
        $db = DB::getInstance();
        $units = $config->extraSettings->buyTroops['packages'][$race][$packageId]['units'];
        $coins = $config->extraSettings->buyTroops['packages'][$race][$packageId]['coins'];
        if (GoldHelper::decreaseGold(Session::getInstance()->getPlayerId(), $coins)) {
